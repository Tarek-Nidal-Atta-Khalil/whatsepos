...
formularBox.innerHTML = `
  ...
  <div class="form-feld">
    ...
    <input id="novumLemma" type="text" placeholder="z. B. puella, bonus, servus">
  </div>
...
async function salvaNovumVerbum(event) {
  ...
  try {
    ...
    if (pars === 'verbum') {
      ...
      const praesens = document.getElementById('novumPraesens').value.trim() || lemma;
      const perfectum = document.getElementById('novumPerfectum').value.trim();
      const supinum = document.getElementById('novumSupinum').value.trim();
      const formae = [
        { forma: normalisiere(praesens), tempus: 'praesens' },
        { forma: normalisiere(perfectum), tempus: 'perfectum' },
        { forma: normalisiere(supinum), tempus: 'supinum' }
      ].filter(function (x) { return x.forma; }).map(function (x) {
        return { forma: x.forma, lemma: normalisiere(lemma), pars_orationis: 'verbum', tempus: x.tempus, flexio_classis: coniugatio, generata: false, irregularis: coniugatio === 'irregularis' };
      });
