...
.adde-uerbum-panel{width:min(100%,980px);margin:0 auto 40px;padding:28px;background:white;border:1px solid #e5e7eb;border-radius:28px;box-shadow:0 14px 34px rgba(0,0,0,.08);text-align:left}.adde-uerbum-panel[hidden]{display:none}.adde-uerbum-panel h2{margin:0 0 18px;font-size:34px}.adde-uerbum-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}.adde-uerbum-verbum{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px;margin-top:18px;padding-top:18px;border-top:1px solid #e5e7eb}.adde-uerbum-verbum[hidden]{display:none}.adde-uerbum-field label{display:block;margin-bottom:6px;font-weight:700;color:#374151}.adde-uerbum-field input,.adde-uerbum-field select{width:100%;max-width:none;margin:0;border:2px solid #d1d5db;border-radius:16px;background:white}.adde-uerbum-actions{display:flex;gap:12px;justify-content:flex-end;margin-top:20px}.adde-uerbum-actions button{border-radius:16px}.adde-uerbum-primary{border:none;background:#111827;color:white}.adde-uerbum-secondary{border:2px solid #d1d5db;background:white;color:#374151}.adde-uerbum-status{min-height:1.4em;margin-top:14px;color:#374151}
@media(max-width:900px){.vocabularium-search-row{flex-direction:column}.vocabularium-admin{width:100%}.vox-button{width:100%}.adde-uerbum-grid,.adde-uerbum-verbum{grid-template-columns:1fr}}
`;
...
addePanel.innerHTML = `
  ...
  <div id="addeLeitformen" class="adde-uerbum-verbum" hidden>
    <div class="adde-uerbum-field"><label for="addePraesens">Praesens</label><input id="addePraesens" type="text" placeholder="canō"></div>
    ...
    <div class="adde-uerbum-field"><label for="addePerfectum">Perfectum</label><input id="addePerfectum" type="text" placeholder="cecinī"></div>
...
function sineMacris(textus) {
  return String(textus || '')
    .replace(/[āăáàâä]/g, 'a').replace(/[ēĕéèêë]/g, 'e')
    .replace(/[īĭíìîï]/g, 'i').replace(/[ōŏóòôö]/g, 'o')
    .replace(/[ūŭúùûü]/g, 'u').replace(/[ȳýỳŷÿ]/g, 'y')
    .replace(/[ĀĂÁÀÂÄ]/g, 'A').replace(/[ĒĔÉÈÊË]/g, 'E')
    .replace(/[ĪĬÍÌÎÏ]/g, 'I').replace(/[ŌŎÓÒÔÖ]/g, 'O')
    .replace(/[ŪŬÚÙÛÜ]/g, 'U').replace(/[ȲÝỲŶŸ]/g, 'Y');
}
...
function aperiLemma(lemma) {
  if (lemma) window.location.href = `lemma.html?lemma=${encodeURIComponent(lemma)}`;
}
...
async function quaereSuggestiones() {
  ...
  data.forEach(item => {
    ...
    lemmaStrong.textContent = formaSupabaseSignata({ ...item, forma: item.lemma }) || item.lemma;
...
async function speichereAddeFormular() {
  ...
  if (pars === 'verbum') {
    statusAdde('Leitformen sind erfasst. Automatische Paradigmenspeicherung folgt als nächster Schritt.');
    return;
  }
...
if (input) {
  ...
  input.addEventListener('keydown', event => {
    ...
    if (event.key === 'Enter') {
      ...
      if (buttons.length > 0) { if (suggestioSelectaIndex < 0) setzeSuggestioSelecta(0); oeffneSuggestioSelecta(); }
