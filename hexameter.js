...
export function normalisiereLatein(textus) {
  return textus.toLowerCase()
    .replace(/[āáàâäǎă]/g, "a").replace(/[ēéèêëĕ]/g, "e").replace(/[īíìîïĭ]/g, "i")
    .replace(/[ōóòôöŏ]/g, "o").replace(/[ūúùûüŭ]/g, "u").replace(/[ȳýỳŷÿ]/g, "y")
...
function erstelleSyllaba(strom, start, ende, ambigua = false) { const textusSyllabae = strom.slice(start, ende + 1); const aperta = estVokalInTextu(textusSyllabae, textusSyllabae.length - 1); return { textus: textusSyllabae, start, ende, aperta, ambigua_muta_cum_liquida: ambigua, quantitas: hatDiphthongum(textusSyllabae) ? "longa_natura_diphthongo" : ambigua ? "ambigua_muta_cum_liquida" : aperta ? "brevis_provisoria" : "longa_positione_provisoria" }; }
...
function erzeugeSilbenVariantenRekursiv(strom, kerne, kernIndex, silbenStart, bisherigeSilben, varianten) {
  ...
  if (istMutaCumLiquida(konsonanten)) { const offeneEnde = kern.ende; const geschlosseneEnde = zwischenStart; erzeugeSilbenVariantenRekursiv(strom, kerne, kernIndex + 1, offeneEnde + 1, [...bisherigeSilben, erstelleSyllaba(strom, silbenStart, offeneEnde, true)], varianten); erzeugeSilbenVariantenRekursiv(strom, kerne, kernIndex + 1, geschlosseneEnde + 1, [...bisherigeSilben, erstelleSyllaba(strom, silbenStart, geschlosseneEnde, true)], varianten); return; }
  let silbenEnde;
...
function indicesSyllabarumPerNucleosInSegmento(silben, segmentum) {
  const indices = [];
  silben.forEach(function(syllaba, index) {
    const indexKern = indexKernInSyllaba(syllaba);
    if (indexKern >= segmentum.start && indexKern <= segmentum.ende) indices.push(index);
  });
  return indices;
}
...
function applicaQuantitatesLexicalesMitFormis(silben, vorbereitet, formaeSelectae) {
  ...
  for (const segmentum of vorbereitet.wortSegmente || []) {
    ...
    if (!forma) continue;
    const indices = indicesSyllabarumPerNucleosInSegmento(resultatum, segmentum);
...
function formaeCompatibilesSegmenti(segmentum, silben) {
  ...
  const numerusSyllabarum = indicesSyllabarumPerNucleosInSegmento(silben, segmentum).length;
...
export function trenneSilbenVariantenVers(textus) {
  ...
  const resultata = [];
  varianten.forEach(function(silben, indexVariante) { const formaeCombinationes = combinaFormas(vorbereitet.wortSegmente || [], silben); formaeCombinationes.forEach(function(formaeSelectae, indexFormae) { const silbenCumLexico = applicaQuantitatesLexicalesMitFormis(silben, vorbereitet, formaeSelectae); resultata.push({ index: resultata.length, indexSyllabarum: indexVariante, indexFormae, schema: silbenCumLexico.map(s => s.textus).join("-"), silben: silbenCumLexico, formaeSelectae }); }); });
  return resultata;
}
