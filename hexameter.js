...
function indicesSyllabarumPerNucleosInSegmento(silben, segmentum) { const indices = []; silben.forEach(function(syllaba, index) { const indexKern = indexKernInSyllaba(syllaba); if (indexKern >= segmentum.start && indexKern <= segmentum.ende) indices.push(index); }); return indices; }
function applicaQuantitatesLexicalesMitFormis(silben, vorbereitet, formaeSelectae) { const resultatum = silben.map(s => ({ ...s })); for (const segmentum of vorbereitet.wortSegmente || []) { const forma = formaeSelectae.get(segmentum.index); if (!forma) continue; const indices = indicesSyllabarumPerNucleosInSegmento(resultatum, segmentum); const quantitates = String(forma.quantitates || "").toUpperCase(); if (quantitates.length !== indices.length) continue; indices.forEach(function(indexSyllabae, offset) { const quantitas = quantitasAusSiglo(quantitates[offset]); if (quantitas) resultatum[indexSyllabae].quantitas = quantitas; }); } return resultatum; }
function formaeCompatibilesSegmenti(segmentum, silben) { const clavis = clavisFormae(segmentum.wort); const formae = formaePerFormam.get(clavis) || []; if (formae.length === 0) return [null]; const numerusSyllabarum = indicesSyllabarumPerNucleosInSegmento(silben, segmentum).length; const compatibiles = formae.filter(function(forma) { const quantitates = String(forma.quantitates || "").toUpperCase(); return quantitates.length === numerusSyllabarum; }); return compatibiles.length > 0 ? compatibiles : [null]; }
function combinaFormasRekursiv(segmenta, silben, index, selectae, resultata) { if (index >= segmenta.length) { resultata.push(new Map(selectae)); return; } const segmentum = segmenta[index]; const formae = formaeCompatibilesSegmenti(segmentum, silben); for (const forma of formae) { if (forma) selectae.set(segmentum.index, forma); else selectae.delete(segmentum.index); combinaFormasRekursiv(segmenta, silben, index + 1, selectae, resultata); } }
...
function silbenAusFormaeOderRegula(vorbereitet, strom) { const resultata = [[]]; for (const segmentum of vorbereitet.wortSegmente || []) { const formae = formaePerFormam.get(clavisFormae(segmentum.wort)) || []; const cumSyllabae = formae.map(f => ({ forma: f, silben: erzeugeSilbenAusLexico(strom, segmentum, f) })).filter(x => x.silben); if (cumSyllabae.length === 0) return null; const nova = []; for (const basis of resultata) for (const x of cumSyllabae) nova.push([...basis, ...x.silben]); resultata.splice(0, resultata.length, ...nova); } return resultata; }
export function trenneSilbenVariantenVers(textus) { const vorbereitet = bereiteVersstromVor(textus); const strom = vorbereitet.versstrom; const lexSilben = silbenAusFormaeOderRegula(vorbereitet, strom); if (lexSilben) return lexSilben.map((silben, index) => ({ index, indexSyllabarum: index, indexFormae: index, schema: silben.map(s => s.textus).join("-"), silben, formaeSelectae: new Map() })); const kerne = findeSilbenkerne(strom); const varianten = []; if (kerne.length === 0) return []; erzeugeSilbenVariantenRekursiv(strom, kerne, 0, 0, [], varianten); const resultata = []; varianten.forEach(function(silben, indexVariante) { const formaeCombinationes = combinaFormas(vorbereitet.wortSegmente || [], silben); formaeCombinationes.forEach(function(formaeSelectae, indexFormae) { const silbenCumLexico = applicaQuantitatesLexicalesMitFormis(silben, vorbereitet, formaeSelectae); resultata.push({ index: resultata.length, indexSyllabarum: indexVariante, indexFormae, schema: silbenCumLexico.map(s => s.textus).join("-"), silben: silbenCumLexico, formaeSelectae }); }); }); return resultata; }
...
function quantitasGraphica(syllaba) { if (syllaba.quantitas === "longa_natura_lexico") return "longa"; if (syllaba.quantitas === "brevis_natura_lexico") return "brevis"; if (syllaba.quantitas === "longa_natura_m_coda") return "longa"; return quantitasSimplex(syllaba); }
...
function notaSyllabamQuantitate(syllaba, quantitas) { const textus = syllaba.textus; const indexDiphthongi = indexDiphthongiInTextu(textus); const indexVocalis = indexPrimiVocalisInTextu(textus); if (indexDiphthongi >= 0) return textus.slice(0, indexDiphthongi) + notaDiphthongumLongum(textus, indexDiphthongi) + textus.slice(indexDiphthongi + 2); if (indexVocalis < 0) return textus; if (syllaba.quantitas === "longa_positione_provisoria") return notaSyllabamLongamPositione(textus, indexVocalis); return textus.slice(0, indexVocalis) + litteraQuantitateNotata(textus[indexVocalis], quantitas) + textus.slice(indexVocalis + 1); }
...
function resolvePedesRekursiv(silbae, pesIndex, initium, pedes) { if (pesIndex === 6) return initium === silbae.length ? pedes : null; const schemata = []; if (pesIndex <= 3) { schemata.push({ nomen: "dactylus", schema: ["longa", "brevis", "brevis"] }); schemata.push({ nomen: "spondeus", schema: ["longa", "longa"] }); } else if (pesIndex === 4) schemata.push({ nomen: "dactylus", schema: ["longa", "brevis", "brevis"] }); else schemata.push({ nomen: "finalis", schema: ["longa", "anceps"] }); for (const schemaInfo of schemata) { if (!pesCompatibilis(silbae, initium, schemaInfo.schema)) continue; const finis = initium + schemaInfo.schema.length; const resultatum = resolvePedesRekursiv(silbae, pesIndex + 1, finis, [...pedes, { index: pesIndex + 1, nomen: schemaInfo.nomen, initium, finis, silbae: silbae.slice(initium, finis) }]); if (resultatum) return resultatum; } return null; }
export function analysiereHexameterPedes(textus) { const analyse = analysiereSilbenVorlaeufig(textus); const conatus = []; for (const variante of analyse.varianten || []) { const pedes = resolvePedesRekursiv(variante.silben, 0, 0, []); conatus.push({ varians: variante.index, schema: variante.schema, successit: Boolean(pedes), pedes: pedes || [], formaeSelectae: variante.formaeSelectae }); if (pedes) return { successit: true, varians: variante.index, schema: variante.schema, silben: variante.silben, pedes, conatus, formaeSelectae: variante.formaeSelectae }; } return { successit: false, varians: null, schema: "", silben: [], pedes: [], conatus }; }
