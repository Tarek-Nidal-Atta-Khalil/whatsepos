...
function trenneSilbenWortFallback(strom, segmentum) { const teil = strom.slice(segmentum.start, segmentum.ende + 1); const kerne = findeSilbenkerne(teil).map(k => ({ ...k, start: k.start + segmentum.start, ende: k.ende + segmentum.start })); const varianten = []; if (kerne.length === 0) return []; erzeugeSilbenVariantenRekursiv(strom, kerne, 0, segmentum.start, [], varianten); return varianten[0] || []; }
