...
export function analysiereHexameterPedes(textus) { const analyse = analysiereSilbenVorlaeufig(textus); const conatus = []; const variantes = [...(analyse.varianten || [])].sort((a, b) => optimaComparatio(a, b)); for (const variante of variantes) { const pedes = resolvePedesRekursiv(variante.silben, 0, 0, []); conatus.push({ varians: variante.index, schema: variante.schema, successit: Boolean(pedes), pedes: pedes || [], formaeSelectae: variante.formaeSelectae, silben: variante.silben }); if (pedes) return { successit: true, varians: variante.index, schema: variante.schema, silben: variante.silben, pedes, conatus, formaeSelectae: variante.formaeSelectae }; } return { successit: false, varians: null, schema: "", silben: [], pedes: [], conatus }; }
...
function numerusTriumBreuium(silbae) { let numerus = 0; for (let i = 0; i <= silbae.length - 3; i += 1) { const tres = silbae.slice(i, i + 3); if (tres.every(s => quantitasSimplex(s) === "brevis")) numerus += 1; } return numerus; }
...
function scoreVariante(variante) { const silben = variante?.silben || []; let score = 0; score += finesPedumProvisorii(silben).size * 100; silben.forEach(function(syllaba) { if (syllaba.quantitas === "longa_natura_lexico") score += 6; else if (syllaba.quantitas === "brevis_natura_lexico") score += 6; else if (syllaba.quantitas === "longa_natura_diphthongo") score += 3; else if (syllaba.quantitas === "longa_natura_m_coda") score += 3; else if (syllaba.quantitas === "longa_positione_provisoria") score += 1; }); return score; }
function optimaComparatio(a, b) { const brevesA = numerusTriumBreuium(a.silben || []); const brevesB = numerusTriumBreuium(b.silben || []); if (brevesA !== brevesB) return brevesA - brevesB; const score = scoreVariante(b) - scoreVariante(a); if (score !== 0) return score; return a.index - b.index; }
function optimaVariante(varianten) { if (!Array.isArray(varianten) || varianten.length === 0) return null; return [...varianten].sort(optimaComparatio)[0]; }
