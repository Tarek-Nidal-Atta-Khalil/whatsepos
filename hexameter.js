...
function silbeAusProfilTeil(strom, start, teil, quantitasSiglum) {
...
function ersetzeSegmentDurchProfil(strom, silben, segmentum, profil) {
  ...
  silben.forEach(function(syllaba, index) {
    const kern = indexKernInSyllaba(syllaba);
    if (kern >= segmentum.start && kern <= segmentum.ende) {
  ...
  for (let i = 0; i < startIndex; i += 1) vor.push(silben[i]);
...
// Supabase-Profile dürfen die interne i/u/v-Behandlung übersteuern.
  ...
  const profilSilben = profil.partesInternae.map(function(teil, offset) {
    const syllaba = silbeAusProfilTeil(
      strom,
      cursor,
      teil,
      profil.quantitates[offset]
    );
    cursor = syllaba.ende + 1;
    return syllaba;
  });

  return [...vor, ...profilSilben, ...nach];
}

function variantesCumProfilibus(strom, silben, vorbereitet) {
  let resultata = [silben];

  for (const segmentum of vorbereitet.wortSegmente || []) {
    const formae = formaePerFormam.get(clavisFormae(segmentum.wort)) || [];
    if (formae.length === 0) continue;

    const profile = formae.map(function(forma) {
      let partes = normalisiereSyllabaeLexico(forma.syllabae)
        .split(".")
        .filter(Boolean);
    ...
    const neueResultata = [...resultata];

    for (const basis of resultata) {
      for (const profil of profile) {
        const ersetzt = ersetzeSegmentDurchProfil(strom, basis, segmentum, profil);
        if (ersetzt) neueResultata.push(ersetzt);
      }
    }

    resultata = neueResultata;
  }
...
function formaeCompatibilesSegmenti(segmentum, silben) {
  if (!Array.isArray(silben)) return [null];
  const clavis = clavisFormae(segmentum.wort); 
  const formae = formaePerFormam.get(clavis) || []; if (formae.length === 0) return [null]; const numerusSyllabarum = indicesSyllabarumPerNucleosInSegmento(silben, segmentum).length; const compatibiles = formae.filter(function(forma) { const quantitates = String(forma.quantitates || "").toUpperCase(); return quantitates.length === numerusSyllabarum; }); return compatibiles.length > 0 ? compatibiles : [null]; }
